use devi::scheduling::{CorePolicy, create_pool_for_policy};
...
pub fn test_single_policy(policy: CorePolicy, num_threads: usize, mixed_ratio: f32) -> BenchmarkStats {
    ...
    if matches!(policy, CorePolicy::Mixed) {
        println!("  Mixed ratio: {:.2} ({} fast, {} efficient)", mixed_ratio, (num_threads as f32 * mixed_ratio) as usize, num_threads - (num_threads as f32 * mixed_ratio) as usize);
    }
...
fn main() {
    ...
    println!("\n=== Key Results ===");
    println!("FastBias is {:.2}x faster than EfficientBias", efficient_time / fast_time);
    
    if mixed_time > 0.0 {
        println!("Mixed (75%) is {:.2}x faster than EfficientBias", efficient_time / mixed_time);
        println!("Mixed (75%) is {:.2}x slower than FastBias", mixed_time / fast_time);
