...
fn test_perft_regression_gate() {
    ...
    assert_eq!(perft(&mut board, 4), 197_281, "Perft(4) regression detected!");
}
...
fn test_search_smoke_test() {
    ...
    assert!(score.abs() < 200, "Starting position score too extreme: {}", score);
...
fn test_evaluation_starting_position() {
    ...
    assert_eq!(evaluate(&board), 0, "Starting position should have equal material");
}