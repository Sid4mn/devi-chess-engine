...
fn test_move_unmove_symmetry() {
    ...
    assert_eq!(perft(&mut board, 3), original_perft_3, "Move-unmove broke position state");
    
    let nc3 = Move::new(Square(1), Square(18), None, None);
    let undo_nc3 = board.make_move(&nc3);
    board.unmake_move(&nc3, undo_nc3);
    
    assert_eq!(perft(&mut board, 3), original_perft_3, "Nc3 move-unmove broke position state");
}