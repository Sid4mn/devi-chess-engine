...
use devi::search::{minimax::alphabeta, parallel_search, search};
...
fn test_recovery_consistency_across_panic_positions() {
    ...
    for panic_at in [0, 5, 10, 15] {
        ...
        assert_eq!(
            baseline_mv.to_algebraic(),
            mv.to_algebraic(),
            "Move changed at panic_at={}",
            panic_at
        );
        assert_eq!(
            baseline_score, score,
            "Score changed at panic_at={}",
            panic_at
        );
    }
}
...
fn test_recovery_stress_depths() {
    ...
    for depth in [4, 5, 6] {
        ...
        assert_eq!(
            baseline_mv.to_algebraic(),
            recovery_mv.to_algebraic(),
            "Depth {}: move changed",
            depth
        );
        assert_eq!(
            baseline_score, recovery_score,
            "Depth {}: score changed",
            depth
        );
    }
...
fn test_single_thread_determinism() {
    ...
    for i in 1..results.len() {
        assert_eq!(
            results[0].0.to_algebraic(),
            results[i].0.to_algebraic(),
            "Non-deterministic: run {} vs run 0",
            i
        );
